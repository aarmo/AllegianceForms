...
namespace AllegianceForms.Engine
{
    ...
    {
        public static Color ShiftHue(Color oldColour, int shiftAmount)
        {
            var a = oldColour.A;
            var h = (oldColour.GetHue() + shiftAmount) % 360;
            var s = oldColour.GetSaturation();
            var l = oldColour.GetBrightness();

            return FromHsl(a, h, s, l);
        }

        public static Color AdjustAlpha(this Color color, float percent)
        {
            var a = (int)(255 * percent);
            var h = color.GetHue();
